...
<div class="space-y-6">
    <div>
        ...
        <p class="mt-1 text-sm text-slate-500">
            Ringkasan omset, modal, profit, dan margin keuntungan dari pesanan gudang.
        </p>
    </div>
    ...
    <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 animate-fade-up">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Modal (COGS)</p>
    ...
    <div class="glass-panel rounded-[24px] border border-white/50 p-6 shadow-lg shadow-slate-200/50">
        <form action="{{ route('admin.sales.index') }}" method="GET" class="grid gap-4 md:grid-cols-4 items-end">
            <div>
            ...
            <div>
                <label for="warehouse_id" class="mb-2 block text-xs font-bold text-slate-600 uppercase tracking-wider">Gudang</label>
                <select
                    id="warehouse_id"
                    name="warehouse_id"
                    class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-slate-900 focus:ring-4 focus:ring-slate-200"
            ...
            <div class="flex gap-2">
                <button
                    type="submit"
                    class="flex-1 inline-flex items-center justify-center rounded-2xl bg-slate-900 px-5 py-3.5 text-sm font-semibold text-white shadow-lg transition hover:bg-slate-800"
                >
                    Filter
                </button>
                <a
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full border-separate border-spacing-y-3">
                ...
                @forelse ($orders as $order)
                    <tbody x-data="{ open: false }">
                        ...
                        <tr x-show="open" x-cloak class="bg-slate-50/70">
                            <td colspan="8" class="px-6 py-4 rounded-2xl border border-slate-200/60 shadow-inner">
                ...
                @empty
                    <tbody>
                        <tr>
                            <td colspan="8" class="px-4 py-12 text-center text-sm text-slate-500 bg-white rounded-2xl shadow-sm border border-slate-200">
